(function () {
  ...
  var WALLET_CHAIN_ID = 'Do-Chain'
  var LEGACY_WALLET_CHAIN_ID = CHAIN_ID
  var WALLET_CHAIN_IDS = [LEGACY_WALLET_CHAIN_ID, WALLET_CHAIN_ID, CHAIN_ID]
  var DENOM = 'udo'
  ...
  function readBridgePayload() {
    var bridge = safeJson(window.localStorage.getItem(WALLET_BRIDGE_KEY), null)
  ...
  function readWallet() {
    ...
    var wallet = bridgeIsExtension && isObject(bridgeWallet) ? bridgeWallet : userWallet || bridgeWallet
    return isObject(wallet) ? wallet : null
  }
  ...
  function findKnownWalletChainID(value, depth) {
    ...
    if (typeof value === 'string') {
      var text = value.trim()
      return WALLET_CHAIN_IDS.indexOf(text) >= 0 ? text : ''
    }
    if (Array.isArray(value)) {
  ...
  function installStyles() {
    ...
    style.textContent = [
      ...
      '.dochain-mfa-eyebrow{margin:0 0 8px;color:#f6b44b;font-size:12px;font-weight:900;text-transform:uppercase}',
      ...
      '.dochain-mfa-field span{font-size:12px;color:#bda9e5;font-weight:800;text-transform:uppercase}',
      ...
      '.dochain-mfa-primary,.dochain-mfa-secondary{border-radius:8px;padding:10px 13px;font:inherit;font-weight:900;cursor:pointer}',
      ...
      '.dochain-mfa-link{color:#ffd28a;font-weight:800;text-decoration:none}',
      ...
      '@media (max-width:760px){.dochain-mfa-grid{grid-template-columns:1fr}.dochain-mfa-head{padding:18px}.dochain-mfa-body{padding:18px}.dochain-mfa-codes{grid-template-columns:1fr}.dochain-mfa-title{font-size:23px}.dochain-mfa-qr{align-items:flex-start;flex-direction:column}.dochain-mfa-identity{padding-right:0}.dochain-mfa-button{position:static;margin-top:10px}.dochain-mfa-title-fallback{margin-left:0;margin-top:10px}}',
    ].join('')
  ...
  async function checkStatus() {
    ...
    var chainEnabled = window.localStorage.getItem(enabledKey(account)) ? ' On-chain activation has been recorded locally.' : ''
    setStatus(
      status.enrolled
        ? 'MFA service enrollment exists. Recovery codes remaining: ' + status.recovery_codes_remaining + '.' + chainEnabled
        : 'No MFA service enrollment found for this account.' + chainEnabled,
  ...
  function updateButtonLabel() {
    ...
    applyButtonStatus(button, account, mfaStatusCache.account === account ? mfaStatusCache.status : null, true)
  ...
  function applyButtonStatus(button, account, status, checking) {
    ...
    button.textContent = checking ? 'Checking MFA' : 'Activate MFA'
  ...
  function scheduleInject() {
    ...
    scheduleInject.timer = window.setTimeout(injectButton, 150)
  }
  ...
  window.addEventListener('do_wallet_change', scheduleInject)
  window.addEventListener('station_wallet_change', scheduleInject)
  window.addEventListener('storage', function (event) {
    if (!event.key || event.key.indexOf('dochain_mfa_') === 0 || event.key === 'user' || event.key === WALLET_BRIDGE_KEY) {
      scheduleInject()
    }
  })
