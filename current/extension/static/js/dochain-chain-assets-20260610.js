(function () {
  ...
  function seed() {
    ...
    Object.keys(CHAINS).forEach(function (chainID) {
      ...
      customLCD[chainID] = CHAINS[chainID].lcd;
    ...
    [].forEach(function (legacy) {
      ...
      networks = networks.filter(function (chainID) { return chainID !== legacy; });
    });

    enabled = Object.assign({}, enabled, { time: Date.now(), networks: networks });
    ...
    changed = writeJSON("EnabledNetworks", enabled) || changed;
    if (changed) {
      try { window.dispatchEvent(new CustomEvent("do_wallet_chain_assets_update")); } catch (error) {}
    }
  }
  ...
  window.addEventListener("do_wallet_change", seed);
