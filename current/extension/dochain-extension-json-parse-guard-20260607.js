(() => {
  ...
  if (originalParse && originalParse.__doWalletObjectGuard) return;

  const guardedParse = function guardedParse(value, reviver) {
    if (value && typeof value === "object") return value;
    return originalParse.call(JSON, value, reviver);
  };

  Object.defineProperty(guardedParse, "__doWalletObjectGuard", {
    configurable: false,
    enumerable: false,
    value: true,
  });
...
(function () {
  ...
  function collapseEmptyInline(parent) {
    ...
    while (current && current !== document.body && current.id !== "do-wallet") {
